...
//			] );
							...
							<?php if ( !empty( $settings['ma_el_warning_message_text'] ) ) : ?>
								<?php if($settings['ma_el_warning_show'] == 'yes'){?>
									<div class="ma-el-restrict-content-message-text">
										<?php Master_Addons_Helper::jltma_warning_messaage($settings['ma_el_warning_message_text'], 'warning',
											$close_button); ?>
									</div>
								<?php } ?>
							<?php endif; ?>
		...
		public function jltma_restrict_content() {
			...
			<div class="ma-el-restrict-content">
				<?php
					...
					} elseif ( $settings['ma_el_restrict_content_elementor_source'] == 'section' and !empty(
					        $settings['ma_el_restrict_content_saved_section'] )) {
						echo Master_Addons_Helper::jltma_elementor()->frontend->get_builder_content_for_display(
						        $settings['ma_el_restrict_content_saved_section'] );
					} elseif ( $settings['ma_el_restrict_content_elementor_source'] == 'widget' and !empty(
		...
		public function jltma_rc_password(){?>
			...
			<div class="form-group mb-2">
				<input
					type="password"
					name="ma_el_restrict_content_pass"
					class="form-control ma-el-input-pass mr-3 mt-2"
					placeholder="<?php esc_html_e( 'Enter Password', MELA_TD ); ?>" size="20"/>
		...
		public function jltma_rc_enter_age(){
			...
			?>
				<div class="form-group my-1 mr-2">
					...
					<input name="ma_el_ra_year" class="ma_el_ra_year" type="text" placeholder="<?php //echo $settings['ma_el_restrict_age_checkbox_label'];?>" /><br />
				</div>
		<?php }
				...
				<div class="ma-el-restrict-content-fields">
					<form <?php echo $this->get_render_attribute_string( 'form_wrapper' ); ?>>
						<div class="card-body">
							...
							<?php if($settings['ma_el_restrict_content_type'] == 'password' ){ $this->jltma_rc_password(); } ?>

							<?php if($settings['ma_el_restrict_content_type'] == 'math_captcha' ){ $this->jltma_rc_math_captcha(); } ?>

							<?php if($settings['ma_el_restrict_age_type'] == 'age_checkbox' ){ $this->jltma_rc_checkbox(); } ?>

							<?php if($settings['ma_el_restrict_age_type'] == 'button_confirm' ){ $this->jltma_rc_age_button_submit(); } ?>

							<?php if($settings['ma_el_restrict_age_type'] == 'input_age' ){ $this->jltma_rc_input_age(); } ?>

							<?php if($settings['ma_el_restrict_age_type'] == 'enter_age' ){ $this->jltma_rc_enter_age(); } ?>
						...
						<div <?php echo $this->get_render_attribute_string( 'popup_content' ); ?>>
									...
									<p class="mb-0 ma-el-rc-button d-block">
										<a
											...
											data-modal="<?php if( $settings['ma_el_restrict_content_popup_type'] =="buttonfullscreen" || $settings['ma_el_restrict_content_popup_type'] =="windowloadfullscreen"){
												echo "true";
											} else {
												echo 'false';
											}?>"
											data-src="#ma-el-rc-modal-<?php echo $this->get_id();?>"
											data-options='{
	...
	Plugin::instance()->widgets_manager->register_widget_type( new Master_Addons_Restrict_Content() );